...
function App() {
  ...
  const incoming = useMemo(() => data.recipes.filter((r) => r.resultId === selectedMonster?.id).sort((a, b) => a.sourceNo - b.sourceNo), [selectedMonster])
  ...
  const parentResults = useMemo(() => {
    ...
    return data.recipes.filter((r) => {
      ...
      const swapped = refMatches(r.lineageRef, b, p2Family) && refMatches(r.mateRef, a, p1Family)
      return (direct || swapped) && (minPlus === null || r.requiredPlus <= minPlus)
...
function MonsterDetail({ monster, incoming, outgoing, favorites, onToggle, expanded, setExpanded, onSelect }) {
  ...
  return <section className="detail-panel" aria-labelledby="detail-heading"><div className="detail-header"><div className={`family-icon large family-${monster.familyId}`}>{familyIcon(monster.familyId)}</div><div><h1 id="detail-heading" tabIndex="-1">{monster.name}</h1><p>{familyName(monster.familyId)} <span className="data-no">No. {monster.id}</span></p></div><button className={favorites.monsters.has(monster.id) ? 'favorite-button active' : 'favorite-button'} onClick={() => onToggle('monsters', monster.id)} aria-pressed={favorites.monsters.has(monster.id)}><Star size={20} fill={favorites.monsters.has(monster.id) ? 'currentColor' : 'none'} />{favorites.monsters.has(monster.id) ? 'お気に入り登録済み' : 'モンスターをお気に入り'}</button></div><div className="detail-stats"><span>基本MAX Lv <b>{monster.maxLevel}</b></span><span>{monster.flying ? '飛行タイプ' : '地上タイプ'}</span><span>{monster.metal ? 'メタルタイプ' : '通常タイプ'}</span></div><section className="detail-section"><button className="section-title-button" onClick={() => setExpanded(!expanded)} aria-expanded={expanded}><h2>このモンスターを作る配合</h2>{expanded ? <ChevronUp /> : <ChevronDown />}</button>{expanded && (incoming.length ? incoming.slice(0, 12).map((recipe) => <RecipeRow key={recipe.recipeKey + recipe.sourceNo} recipe={recipe} favorites={favorites} onToggle={onToggle} onSelect={onSelect} highlight />) : <p className="muted">このモンスターの配合データはありません。</p>)}</section><section className="detail-section"><h2>このモンスターを親に使う配合</h2>{outgoing.length ? outgoing.slice(0, 8).map((recipe) => <RecipeRow key={recipe.recipeKey + recipe.sourceNo} recipe={recipe} favorites={favorites} onToggle={onToggle} onSelect={onSelect} />) : <p className="muted">該当する配合はありません。</p>}</section></section>
}
...
function RecipeRow({ recipe, favorites, onToggle, onSelect, highlight }) {
  ...
  return <article className={highlight ? 'recipe-row highlight' : 'recipe-row'}><div className="recipe-head"><span>優先順 {recipe.sourceNo}</span>{recipe.sourceNos?.length > 1 ? <span className="muted">原典重複: No.{recipe.sourceNos.join(' / ')}</span> : null}<span>必要＋値 <b>{recipe.requiredPlus}</b></span>{recipe.resultPlusBonus ? <span>結果＋{recipe.resultPlusBonus}</span> : null}</div><div className="equation"><button onClick={() => recipe.lineageRef.kind === 'monster' && onSelect(recipe.lineageRef.id)}><small>血統（自分）</small><strong>{lineage}</strong><em>{recipe.lineageRef.kind === 'family' ? '系統指定' : familyName(monsterById.get(recipe.lineageRef.id)?.familyId)}</em></button><span className="operator">×</span><button onClick={() => recipe.mateRef.kind === 'monster' && onSelect(recipe.mateRef.id)}><small>相手（パートナー）</small><strong>{mate}</strong><em>{recipe.mateRef.kind === 'family' ? '系統指定' : familyName(monsterById.get(recipe.mateRef.id)?.familyId)}</em></button><span className="operator arrow">→</span><button className="result-cell" onClick={() => onSelect(recipe.resultId)}><small>結果</small><strong>{result.name}</strong><em>{familyName(result.familyId)}</em></button></div><button className={favorites.recipes.has(recipe.recipeKey) ? 'recipe-favorite active' : 'recipe-favorite'} onClick={() => onToggle('recipes', recipe.recipeKey)} aria-pressed={favorites.recipes.has(recipe.recipeKey)}><Star size={17} fill={favorites.recipes.has(recipe.recipeKey) ? 'currentColor' : 'none'} />{favorites.recipes.has(recipe.recipeKey) ? 'この配合をお気に入り登録済み' : 'この配合をお気に入り'}</button></article>
}
...
function ParentSearch({ parentOne, parentTwo, plus, setParentOne, setParentTwo, setPlus, results, onSelect, onToggle, favorites }) {
  ...
  return <section className="parent-search"><div className="parent-intro"><h1>親2体から配合を探す</h1><p>血統側と相手を指定すると、優先順に配合候補を表示します。</p></div><div className="parent-controls"><label>血統（自分）<select value={parentOne} onChange={(e) => setParentOne(e.target.value)}>{options.map((m) => <option value={m.id} key={m.id}>{m.name}</option>)}</select></label><span className="operator">×</span><label>相手（パートナー）<select value={parentTwo} onChange={(e) => setParentTwo(e.target.value)}>{options.map((m) => <option value={m.id} key={m.id}>{m.name}</option>)}</select></label><label>配合後＋値（任意）<input type="number" min="0" value={plus} onChange={(e) => setPlus(e.target.value)} placeholder="指定なし" /></label></div><div className="parent-results"><div className="panel-head"><h2>配合候補</h2><span>{results.length} 件</span></div>{results.length ? results.map((recipe) => <RecipeRow key={recipe.recipeKey + recipe.sourceNo} recipe={recipe} favorites={favorites} onToggle={onToggle} onSelect={onSelect} highlight />) : <EmptyState message="条件に一致する配合がありません" action="＋値や親を変更してください" />}</div></section>
}
