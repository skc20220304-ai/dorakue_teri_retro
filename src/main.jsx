...
function App() {
  ...
  const filteredMonsters = useMemo(() => {
    ...
    // 名前検索は系統をまたいで探せるようにする。系統フィルターは
    // 検索語が空のときだけ一覧へ適用する。
    return playableMonsters.filter((monster) => {
      if (q) return normalize(monster.name).includes(q)
      return !familyFilter || monster.familyId === familyFilter
    }).slice(0, 80)
  }, [query, familyFilter])
  ...
  return <div className="app-shell" style={{ overflowX: 'hidden' }}>
    ...
    {page === 'favorites' ? <FavoritesPage tab={favoriteTab} setTab={setFavoriteTab} recipes={favoriteRecipes} monsters={favoriteMonsters} onRecipe={(recipe) => { setSelectedId(recipe.resultId); setPage('search') }} onMonster={(id) => selectMonster(id, { toBreeding: true })} onSelectMonster={selectMonster} onToggle={toggleFavorite} notice={notice} syncState={syncState} /> : <>
